...
function mapExpense(row: any): Expense {
  return {
    ...
    paidFrom: row.paid_from ?? "personal",
...
function expenseToRow(expense: Omit<Expense, "id" | "createdAt" | "receiptUrl"> | Partial<Expense>) {
  return {
    ...
    paid_from: expense.paidFrom ?? "personal",
...
function billToRow(bill: Partial<Bill>) {
  return {
    ...
    payment_proof_url: bill.paymentProofUrl ?? null,
...
function mapRepair(row: any): Repair {
  return {
    ...
    priority: row.priority,
    status: row.status,
    expectedCost: row.expected_cost === null ? undefined : Number(row.expected_cost),
    ...
    relatedHomeItemId: row.related_home_item_id ?? undefined,
    notes: row.notes ?? undefined,
...
function repairToRow(repair: Partial<Repair>) {
  return {
    item_name: repair.itemName,
    room: repair.room,
    problem_description: repair.problemDescription,
    priority: repair.priority,
    status: repair.status,
    expected_cost: repair.expectedCost ?? null,
    actual_cost: repair.actualCost ?? null,
    paid_by: repair.paidBy ?? null,
    responsible_person: repair.responsiblePerson,
    technician_name: repair.technicianName ?? null,
    technician_phone: repair.technicianPhone ?? null,
    warranty_status: repair.warrantyStatus ?? null,
    related_home_item_id: repair.relatedHomeItemId ?? null,
...
function mapHomeItem(row: any): HomeItem {
  return {
    ...
    lastRepairDate: row.last_repair_date ?? undefined,
    totalRepairCost: Number(row.total_repair_cost || 0),
    notes: row.notes ?? undefined,
    createdAt: row.created_at,
  };
}
...
function homeItemToRow(item: Partial<HomeItem>) {
  return {
    ...
    last_repair_date: item.lastRepairDate ?? null,
    total_repair_cost: item.totalRepairCost,
...
function mapShoppingItem(row: any): ShoppingItem {
  return {
    ...
    estimatedPrice: row.estimated_price === null ? undefined : Number(row.estimated_price),
    neededByDate: row.needed_by_date ?? undefined,
    assignedBuyer: row.assigned_buyer ?? undefined,
...
function shoppingItemToRow(item: Partial<ShoppingItem>) {
  return {
    ...
    assigned_buyer: item.assignedBuyer ?? null,
    bought: item.bought,
    notes: item.notes ?? null,
...
async function addRemoteActivity(client: SupabaseClient, actionType: string, description: string, performedBy: ProfileId, relatedType?: string, relatedId?: string) {
  const { data } = await client
    .from("activity_log")
    .insert(activityToRow({ actionType, description, performedBy, relatedType, relatedId }))
    .select()
    .single();
...
async function loadOnlineData(client: SupabaseClient): Promise<AppData> {
  const [expenses, budget, bills, tasks, repairs, homeItems, shoppingItems, activityLog] = await Promise.all([
...
export function useDataStore() {
  ...
  useEffect(() => {
    if (!online && loaded) saveLocalData(data);
  }, [data, loaded, online]);
  ...
  const updateExpense = useCallback((id: string, updates: Partial<Expense>) => {
    if (online && client) void client.from("expenses").update(expenseToRow(updates)).eq("id", id);
    setData((prev) => ({ ...prev, expenses: prev.expenses.map((e) => e.id === id ? { ...e, ...updates } : e) }));
  }, [online, client]);

  const deleteExpense = useCallback((id: string) => {
    if (online && client) void client.from("expenses").delete().eq("id", id);
  ...
  const deleteHomeBudgetTransaction = useCallback((id: string) => {
    if (online && client) void client.from("home_budget_transactions").delete().eq("id", id);
  ...
  const addBill = useCallback((bill: Omit<Bill, "id" | "createdAt" | "paid" | "paidBy" | "paymentDate" | "paymentProofUrl">, by: ProfileId) => {
    ...
    if (online && client) void (async () => {
      const { data: inserted } = await client.from("bills").insert(billToRow(item)).select().single();
      if (inserted) setData((prev) => ({ ...prev, bills: [mapBill(inserted), ...prev.bills] }));
      await addRemoteActivity(client, "bill", `Added bill: ${bill.name}`, by, "bill", inserted?.id);
      await refresh();
    })();
    else setData((prev) => addLocalActivity({ ...prev, bills: [item, ...prev.bills] }, "bill", `Added bill: ${bill.name}`, by, "bill", item.id));
    return item;
  }, [online, client, refresh]);

  const updateBill = useCallback((id: string, updates: Partial<Bill>) => {
    if (online && client) void client.from("bills").update(billToRow(updates)).eq("id", id);
    setData((prev) => ({ ...prev, bills: prev.bills.map((b) => b.id === id ? { ...b, ...updates } : b) }));
  }, [online, client]);

  const deleteBill = useCallback((id: string) => {
    if (online && client) void client.from("bills").delete().eq("id", id);
  ...
  const payBill = useCallback((id: string, paidBy: ProfileId, paymentDate: string) => {
    const updates: Partial<Bill> = { paid: true, paidBy, paymentDate };
    updateBill(id, updates);
    const bill = data.bills.find((b) => b.id === id);
    if (bill) {
      if (online && client) void addRemoteActivity(client, "bill", `Paid bill: ${bill.name} (${bill.amount} EGP)`, paidBy, "bill", id).then(() => refresh());
      else setData((prev) => addLocalActivity(prev, "bill", `Paid bill: ${bill.name} (${bill.amount} EGP)`, paidBy, "bill", id));
    }
  }, [updateBill, data.bills, online, client, refresh]);
  ...
  const addTask = useCallback((task: Omit<Task, "id" | "createdAt" | "status">, by: ProfileId) => {
    ...
    if (online && client) void (async () => { await client.from("tasks").insert(taskToRow(item)); await addRemoteActivity(client, "task", `Added task: ${task.name}`, by, "task", item.id); await refresh(); })();
    else setData((prev) => addLocalActivity({ ...prev, tasks: [item, ...prev.tasks] }, "task", `Added task: ${task.name}`, by, "task", item.id));
    return item;
  }, [online, client, refresh]);

  const updateTask = useCallback((id: string, updates: Partial<Task>) => {
    if (online && client) void client.from("tasks").update(taskToRow(updates)).eq("id", id);
    setData((prev) => ({ ...prev, tasks: prev.tasks.map((t) => t.id === id ? { ...t, ...updates } : t) }));
  }, [online, client]);

  const deleteTask = useCallback((id: string) => {
    if (online && client) void client.from("tasks").delete().eq("id", id);
    setData((prev) => ({ ...prev, tasks: prev.tasks.filter((t) => t.id !== id) }));
  }, [online, client]);

  const markTaskDone = useCallback((id: string, by: ProfileId) => {
    updateTask(id, { status: "done" as TaskStatus });
    const task = data.tasks.find((t) => t.id === id);
    if (task) {
      if (online && client) void addRemoteActivity(client, "task", `Completed task: ${task.name}`, by, "task", id).then(() => refresh());
      else setData((prev) => addLocalActivity(prev, "task", `Completed task: ${task.name}`, by, "task", id));
    }
  }, [updateTask, data.tasks, online, client, refresh]);
  ...
  const addRepair = useCallback((repair: Omit<Repair, "id" | "createdAt" | "status" | "photos">, by: ProfileId) => {
    ...
    if (online && client) void (async () => { await client.from("repairs").insert(repairToRow(item)); await addRemoteActivity(client, "repair", `Reported repair: ${repair.itemName}`, by, "repair", item.id); await refresh(); })();
    else setData((prev) => addLocalActivity({ ...prev, repairs: [item, ...prev.repairs] }, "repair", `Reported repair: ${repair.itemName}`, by, "repair", item.id));
    return item;
  }, [online, client, refresh]);

  const updateRepair = useCallback((id: string, updates: Partial<Repair>) => {
    if (online && client) void client.from("repairs").update(repairToRow(updates)).eq("id", id);
    setData((prev) => ({ ...prev, repairs: prev.repairs.map((r) => r.id === id ? { ...r, ...updates } : r) }));
  }, [online, client]);

  const deleteRepair = useCallback((id: string) => {
    if (online && client) void client.from("repairs").delete().eq("id", id);
  ...
  const addHomeItem = useCallback((item: Omit<HomeItem, "id" | "createdAt" | "totalRepairCost">, by: ProfileId) => {
    ...
    if (online && client) void (async () => { await client.from("home_items").insert(homeItemToRow(full)); await addRemoteActivity(client, "item", `Added home item: ${item.name}`, by, "item", full.id); await refresh(); })();
    else setData((prev) => addLocalActivity({ ...prev, homeItems: [full, ...prev.homeItems] }, "item", `Added home item: ${item.name}`, by, "item", full.id));
    return full;
  }, [online, client, refresh]);

  const updateHomeItem = useCallback((id: string, updates: Partial<HomeItem>) => {
    if (online && client) void client.from("home_items").update(homeItemToRow(updates)).eq("id", id);
    setData((prev) => ({ ...prev, homeItems: prev.homeItems.map((i) => i.id === id ? { ...i, ...updates } : i) }));
  }, [online, client]);

  const deleteHomeItem = useCallback((id: string) => {
    if (online && client) void client.from("home_items").delete().eq("id", id);
  ...
  const addShoppingItem = useCallback((item: Omit<ShoppingItem, "id" | "createdAt" | "bought">, by: ProfileId) => {
    ...
    if (online && client) void (async () => { await client.from("shopping_items").insert(shoppingItemToRow(full)); await addRemoteActivity(client, "shopping", `Added shopping item: ${item.name}`, by, "shopping", full.id); await refresh(); })();
    else setData((prev) => addLocalActivity({ ...prev, shoppingItems: [full, ...prev.shoppingItems] }, "shopping", `Added shopping item: ${item.name}`, by, "shopping", full.id));
    return full;
  }, [online, client, refresh]);

  const updateShoppingItem = useCallback((id: string, updates: Partial<ShoppingItem>) => {
    if (online && client) void client.from("shopping_items").update(shoppingItemToRow(updates)).eq("id", id);
  ...
  const deleteShoppingItem = useCallback((id: string) => {
    if (online && client) void client.from("shopping_items").delete().eq("id", id);
  ...
  const markShoppingBought = useCallback((id: string, by: ProfileId) => {
    updateShoppingItem(id, { bought: true });
    const item = data.shoppingItems.find((i) => i.id === id);
    if (item) {
      if (online && client) void addRemoteActivity(client, "shopping", `Bought: ${item.name}`, by, "shopping", id).then(() => refresh());
      else setData((prev) => addLocalActivity(prev, "shopping", `Bought: ${item.name}`, by, "shopping", id));
    }
  }, [updateShoppingItem, data.shoppingItems, online, client, refresh]);
