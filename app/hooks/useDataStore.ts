...
function normalizeData(data: Partial<AppData> | null | undefined): AppData {
  return {
    ...
    homeBudgetSettings: { ...DEFAULT_HOME_BUDGET_SETTINGS, ...(data?.homeBudgetSettings || {}) },
  };
}
...
function getInitialLocalData(): AppData {
  ...
  try { return normalizeData(JSON.parse(raw)); } catch { return EMPTY_DATA; }
}
...
async function loadOnlineData(client: SupabaseClient): Promise<AppData> {
  ...
  return { expenses: (expenses.data || []).map(mapExpense), homeBudgetTransactions: (budget.data || []).map(mapBudgetTransaction), homeBudgetSettings: settings.data ? mapHomeBudgetSettings(settings.data) : DEFAULT_HOME_BUDGET_SETTINGS, bills: (bills.data || []).map(mapBill), tasks: (tasks.data || []).map(mapTask), repairs: (repairs.data || []).map(mapRepair), homeItems: (homeItems.data || []).map(mapHomeItem), shoppingItems: (shoppingItems.data || []).map(mapShoppingItem), activityLog: (activityLog.data || []).map(mapActivity) };
}
...
export function useDataStore() {
  ...
  const refresh = useCallback(async () => { if (online && client) { setData(await loadOnlineData(client)); setLoaded(true); return; } setData(getInitialLocalData()); setLoaded(true); }, [online, client]);
  ...
  const updateHomeBudgetSettings = useCallback((updates: Partial<HomeBudgetSettings>, by: ProfileId) => {
    const nextSettings: HomeBudgetSettings = { ...data.homeBudgetSettings, ...updates, id: "default", standardMonthlyBudget: Number(updates.standardMonthlyBudget ?? data.homeBudgetSettings.standardMonthlyBudget ?? 0), minimumBalance: Number(updates.minimumBalance ?? data.homeBudgetSettings.minimumBalance ?? 200), updatedBy: by, updatedAt: new Date().toISOString() };
    ...
    if (online && client) void (async () => { await client.from("home_budget_settings").upsert(settingsToRow(nextSettings), { onConflict: "id" }); })();
    return nextSettings;
  ...
  const addExpense = useCallback((expense: Omit<Expense, "id" | "createdAt" | "receiptUrl">, by: ProfileId) => {
    const item: Expense = { ...expense, paidFrom: "personal", id: generateId(), receiptUrl: undefined, createdAt: new Date().toISOString() };
    setData((prev) => addLocalActivity({ ...prev, expenses: [item, ...prev.expenses] }, "expense", `Added personal expense: ${expense.description} (${expense.amount} EGP)`, by, "expense", item.id));
    if (online && client) void (async () => { const { data: inserted } = await client.from("expenses").insert(expenseToRow(item)).select().single(); if (inserted) setData((prev) => ({ ...prev, expenses: prev.expenses.map((expenseItem) => expenseItem.id === item.id ? mapExpense(inserted) : expenseItem) })); })();
    return item;
  }, [online, client]);
  const updateExpense = useCallback((id: string, updates: Partial<Expense>) => { const next = { ...updates, paidFrom: "personal" as const }; setData((prev) => ({ ...prev, expenses: prev.expenses.map((e) => e.id === id ? { ...e, ...next } : e) })); if (online && client) void client.from("expenses").update(expenseToRow(next)).eq("id", id); }, [online, client]);
  const deleteExpense = useCallback((id: string) => { setData((prev) => ({ ...prev, expenses: prev.expenses.filter((e) => e.id !== id) })); if (online && client) void client.from("expenses").delete().eq("id", id); }, [online, client]);

  const addHomeBudgetTransaction = useCallback((transaction: Omit<HomeBudgetTransaction, "id" | "createdAt">, by: ProfileId) => { const item: HomeBudgetTransaction = { ...transaction, id: generateId(), createdAt: new Date().toISOString() }; const actionText = transaction.type === "add" ? "Added to Home Budget" : "Removed from Home Budget"; setData((prev) => addLocalActivity({ ...prev, homeBudgetTransactions: [item, ...prev.homeBudgetTransactions] }, "home-budget", `${actionText}: ${transaction.description} (${transaction.amount} EGP)`, by, "home-budget", item.id)); if (online && client) void (async () => { const { data: inserted } = await client.from("home_budget_transactions").insert(budgetTransactionToRow(item)).select().single(); if (inserted) setData((prev) => ({ ...prev, homeBudgetTransactions: prev.homeBudgetTransactions.map((budgetItem) => budgetItem.id === item.id ? mapBudgetTransaction(inserted) : budgetItem) })); })(); return item; }, [online, client]);
  const deleteHomeBudgetTransaction = useCallback((id: string) => { setData((prev) => ({ ...prev, homeBudgetTransactions: prev.homeBudgetTransactions.filter((t) => t.id !== id) })); if (online && client) void client.from("home_budget_transactions").delete().eq("id", id); }, [online, client]);

  const addBill = useCallback((bill: Omit<Bill, "id" | "createdAt" | "paid" | "paidBy" | "paymentDate" | "paymentProofUrl">, by: ProfileId) => { const item: Bill = { ...bill, id: generateId(), createdAt: new Date().toISOString(), paid: false, paidBy: undefined, paymentDate: undefined, paymentProofUrl: undefined }; setData((prev) => addLocalActivity({ ...prev, bills: [item, ...prev.bills] }, "bill", `Added bill: ${bill.name}`, by, "bill", item.id)); if (online && client) void client.from("bills").insert(billToRow(item)); return item; }, [online, client]);
  const updateBill = useCallback((id: string, updates: Partial<Bill>) => { setData((prev) => ({ ...prev, bills: prev.bills.map((b) => b.id === id ? { ...b, ...updates } : b) })); if (online && client) void client.from("bills").update(billToRow(updates)).eq("id", id); }, [online, client]);
  const deleteBill = useCallback((id: string) => { setData((prev) => ({ ...prev, bills: prev.bills.filter((b) => b.id !== id) })); if (online && client) void client.from("bills").delete().eq("id", id); }, [online, client]);
  const payBill = useCallback((id: string, paidBy: ProfileId, paymentDate: string) => { const updates: Partial<Bill> = { paid: true, paidBy, paymentDate }; updateBill(id, updates); const bill = data.bills.find((b) => b.id === id); if (bill) setData((prev) => addLocalActivity(prev, "bill", `Paid bill: ${bill.name} (${bill.amount} EGP)`, paidBy, "bill", id)); }, [updateBill, data.bills]);

  const addTask = useCallback((task: Omit<Task, "id" | "createdAt" | "status">, by: ProfileId) => { const item: Task = { ...task, id: generateId(), status: "pending", createdAt: new Date().toISOString() }; setData((prev) => addLocalActivity({ ...prev, tasks: [item, ...prev.tasks] }, "task", `Added task: ${task.name}`, by, "task", item.id)); if (online && client) void client.from("tasks").insert(taskToRow(item)); return item; }, [online, client]);
  const updateTask = useCallback((id: string, updates: Partial<Task>) => { setData((prev) => ({ ...prev, tasks: prev.tasks.map((t) => t.id === id ? { ...t, ...updates } : t) })); if (online && client) void client.from("tasks").update(taskToRow(updates)).eq("id", id); }, [online, client]);
  const deleteTask = useCallback((id: string) => { setData((prev) => ({ ...prev, tasks: prev.tasks.filter((t) => t.id !== id) })); if (online && client) void client.from("tasks").delete().eq("id", id); }, [online, client]);
  const markTaskDone = useCallback((id: string, by: ProfileId) => { const task = data.tasks.find((t) => t.id === id); setData((prev) => addLocalActivity({ ...prev, tasks: prev.tasks.map((t) => t.id === id ? { ...t, status: "done" as TaskStatus } : t) }, "task", `Completed task: ${task?.name || "Task"}`, by, "task", id)); if (online && client) void client.from("tasks").update({ status: "done" }).eq("id", id); }, [data.tasks, online, client]);

  const addRepair = useCallback((repair: Omit<Repair, "id" | "createdAt" | "status" | "photos">, by: ProfileId) => { const item: Repair = { ...repair, id: generateId(), status: "new", photos: [], createdAt: new Date().toISOString() }; setData((prev) => addLocalActivity({ ...prev, repairs: [item, ...prev.repairs] }, "repair", `Reported repair: ${repair.itemName}`, by, "repair", item.id)); if (online && client) void (async () => { const { data: inserted, error } = await client.from("repairs").insert(repairToRow(item)).select().single(); if (error) console.error("Failed to save repair", error); if (inserted) setData((prev) => ({ ...prev, repairs: prev.repairs.map((repairItem) => repairItem.id === item.id ? mapRepair(inserted) : repairItem) })); })(); return item; }, [online, client]);
  const updateRepair = useCallback((id: string, updates: Partial<Repair>) => { setData((prev) => ({ ...prev, repairs: prev.repairs.map((r) => r.id === id ? { ...r, ...updates } : r) })); if (online && client) void client.from("repairs").update(repairToRow(updates)).eq("id", id); }, [online, client]);
  const deleteRepair = useCallback((id: string) => { setData((prev) => ({ ...prev, repairs: prev.repairs.filter((r) => r.id !== id) })); if (online && client) void client.from("repairs").delete().eq("id", id); }, [online, client]);

  const addHomeItem = useCallback((item: Omit<HomeItem, "id" | "createdAt" | "totalRepairCost">, by: ProfileId) => { const full: HomeItem = { ...item, id: generateId(), totalRepairCost: 0, createdAt: new Date().toISOString() }; setData((prev) => addLocalActivity({ ...prev, homeItems: [full, ...prev.homeItems] }, "item", `Added home item: ${item.name}`, by, "item", full.id)); if (online && client) void client.from("home_items").insert(homeItemToRow(full)); return full; }, [online, client]);
  const updateHomeItem = useCallback((id: string, updates: Partial<HomeItem>) => { setData((prev) => ({ ...prev, homeItems: prev.homeItems.map((i) => i.id === id ? { ...i, ...updates } : i) })); if (online && client) void client.from("home_items").update(homeItemToRow(updates)).eq("id", id); }, [online, client]);
  const deleteHomeItem = useCallback((id: string) => { setData((prev) => ({ ...prev, homeItems: prev.homeItems.filter((i) => i.id !== id) })); if (online && client) void client.from("home_items").delete().eq("id", id); }, [online, client]);

  const addShoppingItem = useCallback((item: Omit<ShoppingItem, "id" | "createdAt" | "bought">, by: ProfileId) => { const full: ShoppingItem = { ...item, id: generateId(), bought: false, createdAt: new Date().toISOString() }; setData((prev) => addLocalActivity({ ...prev, shoppingItems: [full, ...prev.shoppingItems] }, "shopping", `Added shopping item: ${item.name}`, by, "shopping", full.id)); if (online && client) void client.from("shopping_items").insert(shoppingItemToRow(full)); return full; }, [online, client]);
  const updateShoppingItem = useCallback((id: string, updates: Partial<ShoppingItem>) => { setData((prev) => ({ ...prev, shoppingItems: prev.shoppingItems.map((i) => i.id === id ? { ...i, ...updates } : i) })); if (online && client) void client.from("shopping_items").update(shoppingItemToRow(updates)).eq("id", id); }, [online, client]);
  const deleteShoppingItem = useCallback((id: string) => { setData((prev) => ({ ...prev, shoppingItems: prev.shoppingItems.filter((i) => i.id !== id) })); if (online && client) void client.from("shopping_items").delete().eq("id", id); }, [online, client]);
  const markShoppingBought = useCallback((id: string, by: ProfileId) => { updateShoppingItem(id, { bought: true }); const item = data.shoppingItems.find((i) => i.id === id); if (item) setData((prev) => addLocalActivity(prev, "shopping", `Bought: ${item.name}`, by, "shopping", id)); }, [updateShoppingItem, data.shoppingItems]);
  ...
  const getPaymentSummary = useCallback((month: string) => ["moustafa", "doaa", "ahmed", "sherien"].map((pid) => { const profileId = pid as ProfileId; const totalExpenses = data.expenses.filter((e) => e.paidBy === profileId && e.date.startsWith(month) && (e.paidFrom ?? "personal") === "personal").reduce((s, e) => s + e.amount, 0); const totalBills = data.bills.filter((b) => b.paidBy === profileId && b.paymentDate?.startsWith(month)).reduce((s, b) => s + b.amount, 0); const totalRepairs = data.repairs.filter((r) => r.paidBy === profileId && r.createdAt.startsWith(month)).reduce((s, r) => s + (r.actualCost || 0), 0); return { profileId, totalExpenses, totalBills, totalRepairs, total: totalExpenses + totalBills + totalRepairs }; }), [data]);
