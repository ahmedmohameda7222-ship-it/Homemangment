...
import type { Expense, PaymentSource, ProfileId } from "../lib/types";
...
export default function MoneyPage() {
  ...
  const filteredExpenses = useMemo(() => {
    return data.expenses
      .filter((e) => {
        const paidFrom = e.paidFrom ?? "personal";
        if (categoryFilter !== "all" && e.categoryId !== categoryFilter) return false;
        if (personFilter !== "all" && e.paidBy !== personFilter) return false;
        if (sourceFilter !== "all" && paidFrom !== sourceFilter) return false;
        if (search && !e.description.toLowerCase().includes(search.toLowerCase())) return false;
        return true;
      })
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [data.expenses, categoryFilter, personFilter, sourceFilter, search]);
  ...
  const homeBudgetMonthlyTotal = monthlyExpenses
    .filter((e) => (e.paidFrom ?? "personal") === "home-budget")
          ...
          <div className="bg-cream rounded-2xl border border-warm-gray/60 p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-navy-muted">{formatCurrency(homeBudgetMonthlyTotal)} from Home Budget</p>
              </div>
            </div>
        ...
        <section>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            ...
            >
              ...
              <option value="home-budget">Home Budget</option>
            </select>
          </div>
        </section>
          ...
          {filteredExpenses.length === 0 ? (
            ...
            <div className="space-y-2 stagger-children">
              {filteredExpenses.map((expense) => {
                ...
                const paidFrom = expense.paidFrom ?? "personal";
                const isHomeBudget = paidFrom === "home-budget";
                return (
                  <div
                    key={expense.id}
                    className="w-full flex items-center gap-3 p-4 bg-cream rounded-2xl border border-warm-gray/60 text-left hover:bg-cream-dark transition-colors"
                  >
                    <button
                    ...
                    >
                      <div
                        ...
                        style={{ backgroundColor: isHomeBudget ? "#D8B86F22" : (category?.color || "#6B6B80") + "18" }}
                      >
                        <span style={{ color: isHomeBudget ? "#B89A50" : category?.color || "#6B6B80" }}>
                          {isHomeBudget ? <Wallet size={16} /> : CATEGORY_ICON_MAP[expense.categoryId] || <MoreHorizontal size={16} />}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="mt-1 text-[10px] font-semibold uppercase tracking-wider" style={{ color: isHomeBudget ? "#B89A50" : "#6B6B80" }}>
                          {isHomeBudget ? "Home Budget" : "Personal Money"}
                        </p>
                      </div>
...
function ExpenseModal({ isOpen, onClose, onSubmit, defaultValues, defaultPaidBy, title }: ExpenseModalProps) {
  ...
  const [paidBy, setPaidBy] = useState<ProfileId>(defaultPaidBy || "moustafa");
  const [paidFrom, setPaidFrom] = useState<PaymentSource>("personal");
  ...
  useEffect(() => {
    ...
    setPaidBy(defaultValues?.paidBy || defaultPaidBy || "moustafa");
    setPaidFrom(defaultValues?.paidFrom || "personal");
    setPaymentMethod(defaultValues?.paymentMethod || PAYMENT_METHODS[0]);
    setNotes(defaultValues?.notes || "");
  }, [defaultValues, defaultPaidBy, isOpen]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="text-xs font-medium text-navy-muted uppercase tracking-wider mb-1 block">Paid From</label>
          <select
            value={paidFrom}
            onChange={(e) => setPaidFrom(e.target.value as PaymentSource)}
